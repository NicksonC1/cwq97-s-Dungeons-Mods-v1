 ...
 * @param {Number} distance - How long the raycast should be.
 */
export const getEtherwarpBlock = (useLastPacketPos=true, distance=60) => {
    ...
    if (useLastPacketPos) {
        if (!getLastSentCoord() || !getLastSentLook()) {
            return null
        }

        const lastCoord = getLastSentCoord()
    ...
    // Eye height
    y0 += Player.getPlayer().func_70047_e()

    const lookVec = Vector3.fromPitchYaw(pitch, yaw).multiply(distance)
    const [dx, dy, dz] = lookVec.getComponents()
    
    return simEtherwarp(x0, y0, z0, x0 + dx, y0 + dy, z0 + dz)
}
...
/**
 * Simulates an Etherwarp along the path between the given start and end positions.
 * If a successful block is found, it will return the position of the block that the player would be standing
 * ontop of if they had etherwarped there,
 * If no valid destination is found along the path, the function returns null.
 * @param {Number} x0 
 * @param {Number} y0 
 * @param {Number} z0 
 * @param {Number} x1 
